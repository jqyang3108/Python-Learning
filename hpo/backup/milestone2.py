...
def view_page():
    ...
    if re.search(social, url):
        ...
        req = urllib.request.Request(url)
        rawhtml = urllib.request.urlopen(req)
...
def add_glasses(filename, face_info):
    ...
    glass_img = cv2.imread(sys.path[0]+'/testglass.png') #imporm glasses image
    ...
    scaling_factor = 2.5*abs(centerOfEyes[1][0]-centerOfEyes[0][0])/w
    overlay_glasses = cv2.resize(glass_img,None,fx=scaling_factor,fy=scaling_factor,interpolation=cv2.INTER_AREA)
    x = centerOfEyes[0][0] if centerOfEyes[0][0]<centerOfEyes[1][0] else centerOfEyes[1][0]
    x -= 0.3*overlay_glasses.shape[1]
    y += 1.2*overlay_glasses.shape[0]
    ...
    cv2.imwrite(app.static_folder+"/glassesface.jpg",final_img)

    #cv2.imshow('Lets wear Glasses', final_img)
    #cv2.waitKey()
    #cv2.destroyAllWindows()
    return app.static_folder+"/glassesface.jpg"
...
def test(url):
    ...
    with fetch_images(etree) as filename_to_node:
        ...
        path=copy_profile_photo_to_static(facePhotoName[0])

        glassFace = add_glasses(facePhotoName[0],facePhotoName[1])
        print("glassface  :",glassFace)
